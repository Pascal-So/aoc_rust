	...
	buf.split(sep)
		...
		.filter_map(|(i, entry)| {
			...
			match entry.trim() {
				...
				v => Some(v.parse::<S>().map_err(|_| format!("Cannot parse entry {}: '{}'", entry_nr, entry)))
			}
		})
		.collect()
}
...
pub fn parse_vector_from_file<S: FromStr>(file: impl AsRef<std::path::Path>, sep: u8) -> Result<Vec<S>, String> {
...
pub fn parse_lines_from_file<S: FromStr>(file: impl AsRef<std::path::Path>) -> Result<Vec<S>, String> {
